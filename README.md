...
> This project is a ReactJS version of an another, [HTML-CSS-JavaScript project](https://github.com/szilvia-csernus/map-game), which I created as a milestone project when attending [Code Institute](www.codeintitute.net)'s Full Stack Web Application Developer Course.

Not for public use.

&copy; 2025 Szilvia Csernusne Berczes. All rights reserved.
...
- [Count Visit API](https://github.com/szilvia-csernus/count-visit-api) - My own serverless API, used to keep track of the number of page loads.

- [Redux library version ^8.0.5](https://redux.js.org/) - for state management
...
2. Created an S3 bucket for this project, then uploaded the `build` folder into the newly created bucket.
...
Originally, this project was deployed on `Firebase`. The running project is loading slightly slower then the one on AWS and it's only available in the European region but still can be seen here: https://map-it-aa424.web.app/
